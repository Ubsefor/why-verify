...
/*@
    ...
    @/
    void ecnt_empty_g(Graph *g, int f, int t, int k) {
        if (k > g->ecnt) {
        ...
        if (!g->edges[i].existent) {
            ...
            /*@ ghost
              ...
              /@
                ...
                loop invariant \forall integer j;
                    (0 <= j < l) && (j != i) ==>
                        \at(g->edges[j], _Pre) == \at(g->edges[j], Here);

                loop invariant \forall integer j;
                    (0 <= j < l) && (j != i) ==>
                        count{_Pre}(g, f, t, j, (j + 1)) == count{Here}(g, f, t, j, (j + 1));

                loop invariant (l <= i - 1) ==>
                    count{_Pre}(g,f,t, 0, l + 1) == count{_Pre}(g, f, t, 0, l) + count{_Pre}(g, f, t, l, l + 1);

                loop invariant (l <= i - 1) ==>
                    count{Here}(g,f,t, 0, l + 1) == count{Here}(g, f, t, 0, l) + count{Here}(g, f, t, l, l + 1);

                loop invariant (l <= i) ==>
                        count{_Pre}(g, f, t, 0, l) == count{Here}(g, f, t, 0, l);
                ...
                loop invariant (l > i) ==>
                        count{Here}(g, f, t, 0, l) == (count{_Pre}(g, f, t, 0, l) + 1);

                loop invariant \forall integer j;
                (0 <= j <= l - 1) ==>
                    \forall integer f2, t2;
                        (f2 != f || t2 != t) ==>
                            count{_Pre}(g, f2, t2, 0, j + 1) ==
                                count{_Pre}(g, f2, t2, 0, j) + count{_Pre}(g, f2, t2, j, j + 1);
                ...
                loop invariant \forall integer j;
                    (0 <= j <= l) ==>
                        \forall integer f2, t2;
                            (f2 != f || t2 != t) ==>
                                count{Here}(g, f2, t2, 0, j) ==
                                    count{_Pre}(g, f2, t2, 0, j);
                loop assigns \nothing;
                loop variant g->esize - l;
              @/
                while (l < g->esize) {
                    ++l;
                    //@ assert \forall integer f2, t2; (f2 != f || t2 != t) ==> count{Here}(g, f2, t2, l - 1, l) ==  count{_Pre}(g, f2, t2, l - 1, l);
                    //@ assert \forall integer f2, t2; (f2 != f || t2 != t) ==> count{Here}(g, f2, t2, 0, l) ==  count{Here}(g, f2, t2, 0, l - 1) + count{Here}(g, f2, t2, l - 1, l);
                    //@ assert \forall integer f2, t2; (f2 != f || t2 != t) ==> count{_Pre}(g, f2, t2, 0, l) ==  count{_Pre}(g, f2, t2, 0, l - 1) + count{_Pre}(g, f2, t2, l - 1, l);
                    //@ assert \forall integer f2, t2; (f2 != f || t2 != t) ==> count{Here}(g, f2, t2, 0, l - 1) ==  count{_Pre}(g, f2, t2, 0, l - 1);
                    //@ assert \forall integer f2, t2; (f2 != f || t2 != t) ==> count{Here}(g, f2, t2, 0, l) ==  count{_Pre}(g, f2, t2, 0, l - 1) + count{_Pre}(g, f2, t2, l - 1, l);
                }
            */
            /*@ assert all_count(g, f, t) == (all_count{_Pre}(g, f, t) + 1); */
            return;
        }
    }
}
